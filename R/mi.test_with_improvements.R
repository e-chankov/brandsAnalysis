# The multiple.chiRS2.test() is improved impementation of the MI.test(type = "rs2") from the package MRCV.
# The output of the multiple.chiRS2.test() is modified to visulaisation in shiny application.

MMI.test.modified <- function(data, I, J, add.constant = .5, lodds.calc = FALSE){
  n <- nrow(data)
  if(I != 1L){
  ...
  if (J == 1){
    ...
    if (lodds.calc & r == 2){
      lodds <- log( tab[2,2]*tab[1,1]/(tab[1,2]*tab[2,1]) )
    }
    else{
      lodds <- NA
    }
    
    brand1 <- stri_split_fixed(singleVariableName, " | ", simplify = TRUE)
    brand2 <- stri_split_fixed(colnames(Totals), " | ", simplify = TRUE)
    ### We want the Brand1 will be less (in lexigraphical term) than Brand2 in observed chi.sq results
    if (brand1[1,1] <= brand2[1,1]){
      observed.Chi.sq <- data.table(brand1, brand2,
                              statistics = chisq.test.results$statistic, 
                              lodds.ratio = lodds
      )
    }
    else{
      observed.Chi.sq <- data.table(brand2, brand1,
                                    statistics = chisq.test.results$statistic, 
                                    lodds.ratio = lodds
      )
    }
    setnames(observed.Chi.sq, 1:4,  c("Brand1", "Attribute1", "Brand2", "Attribute2"))
  ...
  chisq.values <- function(x){
    ...
    E <- nn*sum(x)/sum(nn)
    statistics <- sum((x - E)^2 * (1/E + 1/(nn - E)))
    if (lodds.calc & r == 2){
      lodds <- log((nn[1] - x[1])*x[2]/( x[1]*(nn[2]-x[2]) ))
    }
    else{
      lodds <- NA
    }
    return(c(statistics = statistics, lodds.ratio = lodds))
  }
  
  brandWithOneAttribute <- stri_split_fixed(singleVariableName, " | ", simplify = T)
  brandWithAttributes <- stri_split_fixed(colnames(Totals), " | ", simplify = T)
  
  ### We want the Brand1 will be less (in lexigraphical term) than Brand2 in observed chi.sq results
  if (brandWithOneAttribute[1,1] <= brandWithAttributes[1,1]){
    observed.Chi.sq <- data.table(
                            brandWithOneAttribute,
                            brandWithAttributes,
                            t(apply(Totals, 2, chisq.values))
          )
  }
  else{
    observed.Chi.sq <- data.table(
  ...
  X.sq.S.rs2 <- (r-1)*J*sum(observed.Chi.sq$statistics)/sum.Di.HGVGH.eigen.sq 
...
SPMI.test.modified <- function(data, I, J, add.constant = .5, lodds.calc = FALSE){
  ...
  if (sum(excluded.categories) > 0){
    ...
    if (I == 1 | J == 1){
      ...
      return(MMI.test.modified(data[, which(! excluded.categories), with = FALSE], I, J, add.constant, lodds.calc))
    }
  ...
  chisq.values <- function(observed.cells){
    ...
    if (lodds.calc){
      lodds <- log(observed.cells*(totals - m.rows - m.cols + observed.cells)/ 
                   ( (m.rows - observed.cells)*(m.cols - observed.cells) ))
    }
    else{
      lodds <- NA
    }
    ...
    output <- data.table(
      stri_split_fixed(rep(names(T.row), times = length(T.col)), " | ", simplify = T),
      stri_split_fixed(rep(names(T.col), each = length(T.row)), " | ", simplify = T),
      c(statistics),
      c(lodds)
      )
    setnames(output, 1:6, c("Brand1", "Attribute1", "Brand2", "Attribute2", 
                            "statistics", "lodds.ratio"))
    return(output)
  }
  
  observed.Chi.sq <- chisq.values(matrix(GH%*%N.freq, nrow = I, byrow = TRUE))
  X.sq.S.rs2 <- I*J*sum(observed.Chi.sq$statistics)/sum.Di.sigma.eigen.sq 
...
multiple.chiRS2.test <- function(data, I, J, add.constant = .5, lodds.calc = FALSE){
  assertInt(I, lower = 1)
  assertInt(J, lower = 1)
  assertNumber(add.constant, lower = 0)
  assert_logical(lodds.calc)
  assertDataTable(data, any.missing = FALSE, ncols = I+J)
  if (I == 1 | J == 1){
    MMI.test.modified(data, I, J, add.constant, lodds.calc)
  }
  else{
    SPMI.test.modified(data, I, J, add.constant, lodds.calc)
  }
}

# two-sided binomial mid-p test (two-sided method is central)
binomial.mid.p <- function(x, n, p){
  if (x < n*p){
    2.0*pbinom(x, n, p, lower.tail = TRUE) - dbinom(x, n, p)
  }
  else{
    2.0*pbinom(x-1, n, p, lower.tail = FALSE) - dbinom(x, n, p)
  }
}
